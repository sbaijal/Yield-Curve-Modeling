...
**Yield Curve Fitting** — plots of the fitted Nelson-Siegel 
and Cubic Spline curves against observed Treasury yields, 
forward rate term structure, discount function, and 
residual fitting errors across maturities.

**Risk Management** — charts showing the portfolio's 
interest rate risk profile including modified duration 
and DV01 by bond, key rate duration decomposition 
revealing where risk is concentrated along the curve, 
stress test P&L across parallel and non-parallel yield 
shocks.
